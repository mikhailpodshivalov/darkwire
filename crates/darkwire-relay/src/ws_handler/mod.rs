...
use crate::app_state::{
    ConnId, HandshakeFailureReason, InviteCreateError, InviteUseError, MsgSendError,
...
use darkwire_protocol::events::{
    ...
    PrekeyGetRequest, PrekeyPublishRequest, RateLimitScope, SessionEndReason, SessionLeaveRequest,
    SessionStartedEvent,
};
use std::{
...
async fn handle_text_message(
    ...
    match incoming.event_type.as_str() {
        ...
        events::names::E2E_HANDSHAKE_INIT | events::names::E2E_HANDSHAKE_ACCEPT => {
            record_handshake_failure(
                state,
                conn_id,
                incoming.event_type.as_str(),
                HandshakeFailureReason::UnsupportedEvent,
            )
            .await;
            send_error(
                socket,
                ...
                "handshake flow is not enabled yet",
                conn_id,
            )
            .await
        }

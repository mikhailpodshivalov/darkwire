...
use darkwire_protocol::events::{
    self, Envelope, InviteCreateRequest, InviteUseRequest, MsgSendRequest, PrekeyGetRequest,
    SessionLeaveRequest,
};
use futures_util::{stream::SplitSink, SinkExt, StreamExt};
use keys::{
    default_key_file_path, HandshakeRole, InitiatorHandshakeContext, KeyManager,
    ...
    loop {
        tokio::select! {
            ...
            _ = handshake_tick.tick() => {
                if let Some(pending) = bootstrap.pending_initiator.as_ref() {
